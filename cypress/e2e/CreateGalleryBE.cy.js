...
describe("Create Gallery test cases", ()=>{
    ...
    it("Create gallery with invalid URL -missing .jpg, .png, .jpeg",()=>{
        ...
        cy.intercept("POST","https://gallery-api.vivifyideas.com/api/galleries").as("invalidFormat")
        navigation.clickOnCreateGallerySubmitButton();
        cy.wait("@invalidFormat").then((intercept)=>{
    ...
    it("Create gallery with invalid title- less than 2 characters",()=>{
        ...
        cy.intercept("POST","https://gallery-api.vivifyideas.com/api/galleries").as("invalidTitle")
        navigation.clickOnCreateGallerySubmitButton();
        cy.wait("@invalidTitle").then((intercept)=>{
            expect(intercept.response.statusCode).to.eq(422);
            expect(intercept.response.statusMessage).to.eq("Unprocessable Entity");
            expect(intercept.response.body.message).to.eq("The given data was invalid.");
           // cy.log(JSON.stringify(intercept.response))
    ...
    it("Create gallery with invalid title- more than 255 characters",()=>{
        ...
        cy.intercept("POST","https://gallery-api.vivifyideas.com/api/galleries").as("titleError")
        navigation.clickOnCreateGallerySubmitButton();
        cy.wait("@titleError").then((intercept)=>{
            expect(intercept.response.statusCode).to.eq(422);
            expect(intercept.response.statusMessage).to.eq("Unprocessable Entity");
            expect(intercept.response.body.message).to.eq("The given data was invalid.");
           // cy.log(JSON.stringify(intercept.response))
    ...
    it("Create gallery with description  more than 1000 characters",()=>{
        ...
        "https://i.natgeofe.com/n/82fddbcc-4cbb-4373-bf72-dbc30068be60/drill-monkey-01_2x3.jpg")
        cy.intercept("POST","https://gallery-api.vivifyideas.com/api/galleries").as("descriptionError")
        
        navigation.clickOnCreateGallerySubmitButton();
        cy.wait("@descriptionError").then((intercept)=>{
            expect(intercept.response.statusCode).to.eq(422);
            expect(intercept.response.statusMessage).to.eq("Unprocessable Entity");
            expect(intercept.response.body.message).to.eq("The given data was invalid.");
           // cy.log(JSON.stringify(intercept.response))
    ...
    it("Create gallery with wrong URL placement", ()=>{
        createGallery.create("naslov", "https://i.natgeofe.com/n/82fddbcc-4cbb-4373-bf72-dbc30068be60/drill-monkey-01_2x3.jpg","opis");
        navigation.clickOnCreateGallerySubmitButton();
    ...
    it("Create gallery with description field that has less than 1000", ()=>{     
        ...
        cy.wait("@createdGallery").then((intercept)=>{
            ...
            expect(intercept.response.statusMessage).to.eq("Created");
            expect(intercept.response.body.user_id).to.eq(46);
            expect(intercept.response.body.title).to.eq("naslov");
            //cy.log(JSON.stringify(intercept.response))
        })
        cy.url().should("contain","gallery-app.vivifyideas.com");
        general.headerTitle.should("have.text","All Galleries");
    ...
    it("Create gallery with title and description- using unicode", ()=>{     
        ...
        cy.wait("@createdGallery").then((intercept)=>{
            ...
            expect(intercept.response.statusMessage).to.eq("Created");
            expect(intercept.response.body.user_id).to.eq(46);
            expect(intercept.response.body.title).to.eq("naslovŠšš");
            expect(intercept.response.body.description).to.eq("oŠšš");
             //cy.log(JSON.stringify(intercept.response))
        })
        cy.url().should("contain","gallery-app.vivifyideas.com");
        general.headerTitle.should("have.text","All Galleries");
    ...
    it("Create gallery with description and title using digits", ()=>{   
        ...
        cy.wait("@createdGallery").then((intercept)=>{
            ...
            expect(intercept.response.statusMessage).to.eq("Created");
            expect(intercept.response.body.user_id).to.eq(46);
            expect(intercept.response.body.title).to.eq("naslovŠš111");
            expect(intercept.response.body.description).to.eq("222");
             //cy.log(JSON.stringify(intercept.response))
        })
        cy.url().should("contain","gallery-app.vivifyideas.com");
        general.headerTitle.should("have.text","All Galleries");
    ...
    it("Create gallery with png files", ()=>{    
        ...
        cy.wait("@createdGallery").then((intercept)=>{
            ...
            expect(intercept.response.statusMessage).to.eq("Created");
            expect(intercept.response.body.user_id).to.eq(46);
            expect(intercept.response.body.title).to.eq("opisni title");
